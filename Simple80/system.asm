;
; CP/M 2.2 ROM with mini built in ROM disc, second bank RAM disc and
; CF support.
;
; Things to consider
...
SIOACmd		equ 1		;location of SIO command/status reg
SIOBData	equ 2		;location of SIO chan A data
SIOBCmd		equ 3		;location of SIO command/status reg
...
; disk parameter header for disk 03
	...
	dw dirbf,dpblk3
	dw 0,all03
; disk parameter header for disk 04 (ROM)
	dw 0,0
	dw 0,0
	dw dirbf,dpblk4
	dw 0,all04
...
dpblk5:
; RAM disk has a similar DPB
	dw 16		; sectors per track
	db 3		; block shift factor
	db 7		;block mask
	db 0		;null mask
	dw 62		;disk size-1
	dw 31		;32 directory entries should do
	db 080h		;alloc 0 (AL0)
	db 0		;alloc 1 (AL1))
	dw 0		;check size (CKS)
	dw 0		; track offset (OFF)
...
;
;	RAM and ROM disk. There is some magic involved here. The ROM disc
;	works because the ROM and RAM images are identical at these
;	addresses. The RAM one because we loaded a few bytes carefully into
;	both banks at the top
;
...
read_ramrom:
	...
	inc h
	ld de,(dmaad)
	; Now copy 128 bytes via the helper. We can't optimize this case as
	; RAM writes with ROM mapped must match the value at that ROM
	; address
	ld b,128
romsector_copy:
	push bc
...
;	copiers. It also exists at the same address in ROM so we can access
...
;
;
;	Jump table for helpers so they can be used outside of CP/M
;
	org 0ffe0h
put_far:
	jp do_put_far			; FFE0
