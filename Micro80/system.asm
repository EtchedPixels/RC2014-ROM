;
; CP/M 2.2 ROM with second bank RAM disc and CF support.
;
; Things to consider
...
; 4/28/19 by Bill Shen
;
; CP/M 2.2 is (C) Lineo Inc.
;
...
BUSY		equ 7
	...
	org 0da00h
;
;	Really this bit is at but we are relocatable so once we hit main
;	post relocation all is good. Until then beware
;
start:
;	From ROM
	di
	...
	ld a,03h
	out (0xef), a
	; ROM is now only visible below 16K
	ld hl,main-start	; get the address in ROM
	ld de,main
	ld bc,0fffeh-main	; keep fffe/ffff as they are ramdisk magic
	ldir
	; CP/M is now in RAM where we want it
	; We can't turn the ROM off yet
	jp main


main:
	ld a,3		; MCR
	out (0eeh),a	; Force all RAM A16 high
	in a,(0efh)
	and 0fch
	out (0efh),a
	...
	ld a,2		; CSBR
	out (0eeh),a
	ld a,0f0h	; force common F000-FFFF and unavoiably 0000-0FFF

	ld a,3
	out (0efh),a	; so we can just assume MCR is the pointer	
...
welcome:
	...
	db 'Welcome to CP/M 2.2.',13,10,0

	org 0dc00h
;
;	CP/M starts at DC00 (we may well be able to move it up more)
;
;**************************************************************
;*
;*             C P / M   version   2 . 2
...
; Four virtual CF disk interfaces
...
; disk parameter header for disk 03
	...
	dw dirbf,dpblk3
	dw 0,all03
; disk parameter header for disk 04 (RAM)
	dw 0,0
	dw 0,0
	dw dirbf,dpblk4
	dw 0,all05
...
dpblk4:
; DPB for RAM disk (52K) - FIXME could be 56K
;
; Try and make best use of space whilst keeping overheads down
;
	dw 16		;sectors per track	(2K)
	db 3		;block shift factor	(1K blocks)
	db 7		;block mask
	db 0		;extent mask
	dw 51		;disk size-1		(52K)
	dw 31		;32 directory entries is sufficient
	db 080h		;alloc 0 (AL0)	512 bytes of directory
	db 0		;alloc 1 (AL1))
	dw 0		;check size (CKS)
...
;
	...
	out (0eeh),a
	ld a,03h
	out (0efh),a		; Low 16K ROM
	ld a,3
	out (0eeh),a
	in a,(0efh)
	or 1
	out (0efh),a		; ROM reappears in low 16K

	ldir			; mirror back into RAM

	and 0feh
	out (0efh),a		; ROM vanishes
	ld a,2
	out (0eeh),a
	ld a,0f0h
	out (0efh),a		; Restore for RAMdisc
	ld a,3
	out (0efh),a		; Restore for RAMdisc
...
CONST:	ld a,(iobyte)
	and 3
	cp 2			; batch mode
	jr nz, DEVCONST		; rest are all the same
	jr DEVLISTST
...
; compute proper disk parameter header address
	...
	ld a,c
	cp 4
	jr nc,cfselok	; ROM disk and RAM disk (4 and 5)
	;
	;	On first use of CF do the media setup
	;
	ld a,(cfinit)
	or a
	ret z
...
;so we will allow space to set up read command...
	...
	; CF adapter volumes A-D

; check if sector data already existed in buffer
; assuming Little Endian storage
	ld bc,(sector)	; MSB into B LSB into C
	ld a,c		; get LSB sector address into reg A
...
move2DMA:
	...
	jr z,movesec	; if zero then data is in first 128 bytes of buffer
	dec a
	jr z,movesec1	; if 1 then data is in 2nd 128-byte block of buffer
	dec a
	jr z,movesec2	; if 2 then data is in 3rd 128-byte block of buffer
...
readfail:
	ld a,0ffh
	ld (CFLA1623),a	; Invalidate buffer pointer
	ld a,1
	pop bc
	pop de
	pop hl
...
DMA2BUF:
	...
	ld a,(sector)	; get LSB sector address
	and 3		; mask off hig bits
	jr z,DMA2sec	; if zero then data is in first 128 bytes of buffer
	dec a
	jr z,DMA2sec1	; if 1 then data is in 2nd 128-byte block of buffer
	dec a
	jr z,DMA2sec2	; if 2 then data is in 3rd 128-byte block of buffer
...
ram_map:
	; 16 sectors per track of 128 bytes
	; offset is
	; TTTTTSSS SXXXXXXX
	ld a,(track)	; tracks 0-25  (each of 2048 bytes)
	add a		
	add a
	add a		; now in the upper bits where we want it
	ld h,a
	ld a,(sector)
	ld l,0
	rra		; sector/2 is high part of offset
	rr l		; 0 or 80h for sector start (page aligned)
	add h
	add 10h		; offset into alternate bank
	ld h,a		
	ret
...
CFLA815:
	ds 1
CFLA1623:
	ds 1		;;;CF LA address, MSB
CFsecdat0:
	ds 128		;;;CF sector data, 512 bytes
CFsecdat1:
	ds 128
CFsecdat2:
	ds 128
...
lastsel: dw 0ffffh
	...
	org 0ff00h
;
;	Make sure this ends up common. This is a quick hack we should
;	optimize ram read/write to inline this properly with a bounce
;	buffer
;
do_put_far:
	in a,(0efh)
